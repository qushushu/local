...
let store = new Vuex.Store({
	...
	mutations: {
		...
		updateRunInfo1(state,runInfo) {
			let seedling_light = runInfo.plant_light;
			if(seedling_light) {
				state.runInfo.seedling_light_dig = seedling_light.param
			} else {
				state.runInfo.seedling_light_dig = [];
			}
			state.runInfo.plant_light_ana = seedling_light.ana;
			state.plant_dig = [];
				let result = [];
				for(let i in seedling_light.dig) {
					result.push(seedling_light.dig[i].item);
	...
	actions: {
		...
		*/
		...
		updateRunInfo1({commit}) {
			axios({
	            method: 'post',
	            url: `${localMode ? localBaseUrl : (tmpBaseUrl + '/plant')}/la/run/info`,
	            data: {
	    			data: {
					    dev_id: "1"
					}
