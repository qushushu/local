        ...
        <a-card class="card-pd nobdCard">
            <table class="normal-table text-center">
                ...
                <tr>
                    ...
                    <td class="td-name">{{value1}} <a href="javascript:;" class="editText" @click="showJzLayer(1)">{{$t('message.修改')}}</a></td>
                    <td class="td-name">{{ECCurList[0]}}</td>
                    <td class="td-name">{{xishu1}} <a href="javascript:;" class="editText" @click="showXsLayer(1)">{{$t('message.修改')}}</a></td>
                    <td class="td-name"><el-button type="primary" @click="toJz(1)" size="small">{{$t('message.校正')}}</el-button></td>
                </tr>
                <tr>
                    <td class="td-name">EC2</td>
                    <td class="td-name">{{value2}} <a href="javascript:;" class="editText" @click="showJzLayer(2)">{{$t('message.修改')}}</a></td>
                    <td class="td-name">{{ECCurList[1]}}</td>
                    <td class="td-name">{{xishu2}} <a href="javascript:;" class="editText" @click="showXsLayer(2)">{{$t('message.修改')}}</a></td>
                    <td class="td-name"><el-button type="primary" @click="toJz(2)" size="small">{{$t('message.校正')}}</el-button></td>
                </tr>
            </table>
        </a-card>
         ...
         <div>
            <el-form :inline="true">
                <el-form-item :label="$t('message.系数：')">
                    ...
                    <span style="font-size: 12px;color: red">系数需在0.5到1.5之间，最多5位小数</span>
         </div>
          <span slot="footer" class="dialog-footer">
    ...
    export default {
        ...
        watch: {
            signNum(d) {
                ...
                if(d) {
                    if(this.signTip && this.signTip.code && this.signTip.code==200) {
                        this.$message({
                            type: 'success',
        ...
        computed: {
            ...
            xishu2() {
                return this.setXs2 || (this.ECCurList[1] ? (this.value2 / this.ECCurList[1]).toFixed(5) : '--');
            },
            runInfo() {
        ...
        methods: {
            ...
            sureSetXs() {
                if(isNaN(this.showXs)) {
                    this.$message({
                        type: 'error',
                        message: this.$t('message.系数需在0.5-1.5之间!')
                    });
                    return;
                }
