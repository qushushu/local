    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    account: [{ required: true, message: this.$t('message.请输入用户名'), trigger: 'change' }],
                    password: [{ required: true, message: this.$t('message.请输入密码'), trigger: 'change'}]
                },
                editCurrent: {
                    id: 0,
                    account: "",
        ...
        computed: {
            // 层大小
            autoDialogWidth() {return this.isMobile ? "303px" : "50%"}
        },
        methods: {
            // 获取所有用户
            async getAllUser() {
