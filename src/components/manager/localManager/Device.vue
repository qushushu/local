...
<template>
	<div class="ym-main">
		<a-card class="card-pd">
			...
			<!-- 搜索栏、新增 end -->
			<!-- 表格 start -->
			<el-table ref="multipleTable" :data="tableData" border stripe size="small" v-if="!isMobile">
			    ...
			    <el-table-column prop="dev_inst_id" :label="$t('message.设备地址')"></el-table-column>
			...
			<el-table ref="multipleTable" v-if="isMobile" :data="tableData" border stripe size="small">
			    <el-table-column prop="instance_number" :label="$t('message.设备编号')"></el-table-column>
			    <el-table-column prop="model" :label="$t('message.设备型号')"></el-table-column>
