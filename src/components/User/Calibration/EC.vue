<template>
    <div>
        <a-card class="card-pd nobdCard">
            <!-- 步骤 start -->
            <el-steps :active="active" finish-status="success" size="small" :align-center="true" class="space-btm1">
              <el-step :title="$t('message.系数初始化')"></el-step>
              <el-step :title="$t('message.校正')"></el-step>
            </el-steps>
            <!-- 步骤 end -->
            <!-- 第一步 start -->
            <table class="normal-table text-center  space-btm" v-if="active == 0">
                <tr>
                    ...
                    <td class="td-name">{{$t('message.系数初始化')}}</td>
                </tr>
                <tr>
                    <td class="td-name">EC1</td>
                    <td class="td-name">{{xishu1}}</td>
                    <td class="td-name"><el-button type="primary" @click="toJz(1)" size="small" :disabled="btnDis">{{$t('message.系数初始化')}}</el-button></td>
                </tr>
                <tr>
                    <td class="td-name">EC2</td>
                    <td class="td-name">{{xishu2}}</td>
                    <td class="td-name"><el-button type="primary" :disabled="btnDis" @click="toJz(2)" size="small">{{$t('message.系数初始化')}}</el-button></td>
                </tr>
            </table>
            <!-- 第一步 end -->
            <!-- 第二步 start -->
            ...
            <el-row type="flex" justify="space-between">
                ...
                <el-button @click="next" size="small" :disabled="active == 1">{{$t('message.下一步')}}</el-button>
            </el-row>
            <!-- 下一步 上一步 end -->
        </a-card>
        <!-- 校正弹窗 start -->
        <el-dialog :title="'EC' + currentEC + $t('message.校正')" :visible.sync="dialogVisible" :width="loginDialogWidth">
            <el-form :inline="true">
                <el-form-item><el-button @click="setCShowV(currentEC,1413)" :type="showValue == 1413 ? 'primary' : 'default'" class="space-btm" size="small">1413</el-button></el-form-item> 
                <el-form-item><el-button @click="setCShowV(currentEC,3000)" :type="showValue == 3000 ? 'primary' : 'default'" class="space-btm" size="small">3000</el-button></el-form-item>
                <el-form-item><el-input size="small" v-model="showValue" :placeholder="$t('message.请输入内容')"></el-input></el-form-item>
            </el-form>
        ...
        <el-dialog :title="$t('message.系数修改')" :visible.sync="dialogVisible1" :width="loginDialogWidth">
            <el-form :inline="true">
                <el-form-item :label="$t('message.系数：')"><el-input size="small" v-model="showXs" :placeholder="$t('message.请输入系数')"></el-input></el-form-item>
            </el-form>
            <span class="errTipText">系数应介于0.5和1.5之间,小数点后最多5位</span>
            <span slot="footer" class="dialog-footer">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            signTip() {return this.$store.state.signTip},
            xishu1() {
                if(this.active == 0) {
                    return this.setXs1;
                } else {
